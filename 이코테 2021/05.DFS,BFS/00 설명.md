...
**그래프의 한 경로를 끝까지 탐색한 후 백트래킹하며 다른 경로를 탐색하는 알고리즘.** 일반적으로 스택 자료구조 또는 재귀 방식으로 구현한다. 메모리 사용량이 적다는 장점이 있으나, 최단 경로를 보장하지 못한다는 단점이 있다. 데이터 N개 기준으로 탐색 시간은 O(N)의 시간 복잡도를 갖는다.
